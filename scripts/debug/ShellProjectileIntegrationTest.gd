...
func _print_result(
	...
	print(
		(
			"[ShellIntegration] target=%.1f aim=%s velocity=%s pitch=%.2f "
...
func _finish() -> void:
	for failure in _failures:
		push_error("SHELL PROJECTILE INTEGRATION TEST: %s" % failure)
	if _failures.is_empty():
		print("SHELL_PROJECTILE_INTEGRATION_TEST PASS")
