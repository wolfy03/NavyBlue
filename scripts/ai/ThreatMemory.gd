...
func register_damage(
	...
	entry["dynamic_threat"] = float(entry["dynamic_threat"]) + maxf(damage, 0.0)
...
func get_snapshot(candidate: Node, now_sec: float = -1.0) -> Dictionary:
	...
	return {
		...
		"dynamic_threat": float(entry["dynamic_threat"]),
...
func _decay_entry(entry: Dictionary, now_sec: float) -> void:
	...
	entry["dynamic_threat"] = float(entry["dynamic_threat"]) * decay_multiplier
