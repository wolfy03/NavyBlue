...
func _run() -> void:
	...
	for _frame in 90:
		await physics_frame
	var player_turret := player.get_turrets()[0] as Turret
	_check(player_turret.get_target_pitch_degrees() >= 10.0, "5 km target uses a visible gun elevation of at least 10 degrees")
	print(
		"BALLISTIC_5KM target_pitch=%.2f deg effective_gravity=%.2f m/s2" % [
			player_turret.get_target_pitch_degrees(),
			player_turret.get_effective_ballistic_gravity_mps2(),
		]
	)
	...
	print("BATTLEFIELD expansion checks=%d failures=%d" % [35, _failures.size()])
